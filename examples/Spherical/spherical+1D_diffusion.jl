...
anim = @animate for i = LinRange(0, sol_sph.t[end], 100)
    ...
    p1 = plot(distance, Fe0, label="Fe initial", linestyle = :dash, linewidth=1, dpi=200, title = title = @sprintf("Total Time = %.2f Myr | T = %.0f °C | P = %.1f GPa", i*t_charact, T[1].val, P[1].val), legend=:outerbottomright, linecolor=1,xlabel = "Distance from the core (µm)")
